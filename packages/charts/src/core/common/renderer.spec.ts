...
import { DataAccessor, IAccessors, IDataSeries, IPosition, IRendererEventPayload } from "./types";
...
describe("Renderer >", () => {
    ...
    describe("getContainerStateStyles()", () => {
        ...
        it("should decrease opacity for 'deemphasized' state", () => {
            ...
            expect(renderer.getContainerStateStyles(state)).toEqual(expectedStyle);
        });
        ...
        it("should return default styles for no state specified", () => {
            // @ts-ignore: Disabled for testing purposes
            expect(renderer.getContainerStateStyles(null)).toEqual(expectedStyle);
        });
    ...
    describe("getDomain()", () => {
        it("should calculate min and max values, using data accessor with the scale name", () => {
            ...
            const domain = renderer.getDomain(data, { id: "x", data, accessors: { data: { a: accessor } } }, "a", new NoopScale());
        ...
        it("should return the empty continuous domain for null data", () => {
            const accessor: DataAccessor = (d) => d.a;
            // @ts-ignore: Disabled for testing purposes
            const domain = renderer.getDomain(null, { id: "x", data: null, accessors: { data: { a: accessor } } }, "a", new NoopScale());
            expect(domain).toEqual(EMPTY_CONTINUOUS_DOMAIN);
        });

        it("should return the empty continuous domain for empty data", () => {
            const accessor: DataAccessor = (d) => d.a;
            const domain = renderer.getDomain([], { id: "x", data: [], accessors: { data: { a: accessor } } }, "a", new NoopScale());
            expect(domain).toEqual(EMPTY_CONTINUOUS_DOMAIN);
        });
    });
