...
import { DATA_POINT_INTERACTION_RESET, HIGHLIGHT_DATA_POINT_EVENT, STANDARD_RENDER_LAYERS } from "../../constants";
...
import { D3Selection, IAccessors, IChartSeries, IDataPoint, IDataSeries, ILasagnaLayer, InteractionType, IPosition, IRendererEventPayload } from "./types";
...
class MockRenderer<TA extends IAccessors> extends Renderer<TA> {
    ...
    public getDataPointPosition(dataSeries: IDataSeries<IAccessors>, index: number, scales: Scales): IPosition {
...
describe("components >", () => {
    ...
    const getSeriesContainer = (layerName: RenderLayerName, seriesId: string): Selection<SVGElement, any, BaseType, {}> => {
        const dataLayer = svg.selectAll(`g.${Lasagna.LAYER_CLASS}-${layerName}`);
    ...
    describe("render-engine >", () => {
        ...
        describe("update >", () => {
            it("should invoke draw on the renderer for each series", () => {
                ...
                renderEngine.update();
                mockDoubleSeriesSet.forEach(async series => expect(series.renderer.draw).toHaveBeenCalled());
            });

            it("should not invoke draw on the renderer for series with an invalid scale domain", () => {
                const invalidScale = { id: "x", isDomainValid: () => false } as IScale<any>;
                const validScale = { id: "y", isDomainValid: () => true } as IScale<any>;
        ...
        describe("highlightDataPointSubscription >", () => {
            it("should invoke highlightDataPoint on triggering of HIGHLIGHT_DATA_POINT_EVENT", () => {
                ...
                renderEngine.rendererSubject.next({
                    ...
                    data: { seriesId: mockDoubleSeriesSet[1].id, index: 0, data: {}, position: null },
                });
        ...
        describe("updateSeriesContainers >", () => {
            ...
            it("should invoke lasagna.addLayer when an added series needs a new layer", () => {
                ...
                renderEngine.updateSeriesContainers();
                expect(lasagna.addLayer).toHaveBeenCalledWith(STANDARD_RENDER_LAYERS[RenderLayerName.foreground]);
            });

            it("should only include a series container in a layer if it's needed by the series", () => {
                let seriesContainer = getSeriesContainer(RenderLayerName.data, mockDoubleSeriesSet[1].id);
                expect(seriesContainer.nodes().length).toEqual(1);
                seriesContainer = getSeriesContainer(RenderLayerName.background, mockDoubleSeriesSet[1].id);
            ...
            it("should add a series container to the appropriate layer(s) when a series is added", () => {
                ...
                const seriesContainer = getSeriesContainer(RenderLayerName.data, newSeriesSet[2].id);
                expect(seriesContainer.nodes().length).toEqual(1);
            });
            ...
            it("should remove a series container from the appropriate layer(s) when a series is removed", () => {
                ...
                const seriesContainer = getSeriesContainer(RenderLayerName.data, mockSeriesId);
                expect(seriesContainer.nodes().length).toEqual(0);
            });
        });
        ...
        describe("emitInteractionDataPoints >", () => {
            it("should add a data point to the collection of highlighted data points", () => {
                ...
                renderEngine.emitInteractionDataPoints({ interactionType: InteractionType.MouseMove, values: {} });
                expect((<any>renderEngine).highlightedDataPoints[1]).toBeDefined();
            });
            ...
            null data to the collection of highlighted data points`, () => {
                ...
                renderEngine.emitInteractionDataPoints({ interactionType: InteractionType.MouseMove, values: {} });
                expect((<any>renderEngine).highlightedDataPoints[1].index).toEqual(DATA_POINT_INTERACTION_RESET);
                expect((<any>renderEngine).highlightedDataPoints[2]).toBeDefined();
            });
            ...
            it(`should trigger a HIGHLIGHT_DATA_POINT_EVENT and the interactionDataPointsSubject if the dataIndex
                ...
                const getDataPointIndexSpy = spyOn(mockSingleSeriesSet[0].renderer, "getDataPointIndex");
                ...
                expect(renderEngine.rendererSubject.next).toHaveBeenCalledWith(<IRendererEventPayload>{
                    eventName: HIGHLIGHT_DATA_POINT_EVENT,
                    data: expectedDataPoint,
                });
                expect(renderEngine.interactionDataPointsSubject.next).toHaveBeenCalledWith({
                    interactionType: InteractionType.MouseMove,
                    dataPoints: { [mockSingleSeriesSet[0].id]: expectedDataPoint },
                });
            });
            ...
            it(`should not trigger HIGHLIGHT_DATA_POINT_EVENT nor the interactionDataPointsSubject if the dataIndex
                ...
                spyOn(renderEngine.interactionDataPointsSubject, "next");
                renderEngine.emitInteractionDataPoints({ interactionType: InteractionType.MouseMove, values: {} });
                expect(renderEngine.rendererSubject.next).not.toHaveBeenCalled();
                expect(renderEngine.interactionDataPointsSubject.next).not.toHaveBeenCalled();
            });
            ...
            it(`should not trigger HIGHLIGHT_DATA_POINT_EVENT nor the interactionDataPointsSubject if the new dataIndex
                ...
                renderEngine.emitInteractionDataPoints({ interactionType: InteractionType.MouseMove, values: {} });
                renderEngine.emitInteractionDataPoints({ interactionType: InteractionType.MouseMove, values: {} });
                expect(renderEngine.rendererSubject.next).toHaveBeenCalledTimes(1);
                expect(renderEngine.interactionDataPointsSubject.next).toHaveBeenCalledTimes(1);
            });
        ...
        describe("highlightDataPoint >", () => {
            it(`should invoke the highlightDataPoint method of the correct renderer`, () => {
                ...
                expect(mockDoubleSeriesSet[0].renderer.highlightDataPoint).not.toHaveBeenCalled();
                expect(mockDoubleSeriesSet[1].renderer.highlightDataPoint).toHaveBeenCalled();
            });
        });

        describe("setSeriesStates >", () => {
            it(`should apply the correct styling attribute values to the container of the specified series`, () => {
                const seriesContainer = getSeriesContainer(RenderLayerName.data, mockDoubleSeriesSet[1].id);
                expect(seriesContainer.attr("visibility")).toBeNull();
                renderEngine.setSeriesStates([{ seriesId: mockDoubleSeriesSet[1].id, state: RenderState.hidden }]);
            ...
            it(`should invoke renderer.setSeriesState only for the specified series`, () => {
                ...
                renderEngine.setSeriesStates([{ seriesId: mockDoubleSeriesSet[1].id, state: RenderState.hidden }]);
                expect(mockDoubleSeriesSet[0].renderer.setSeriesState).not.toHaveBeenCalled();
                expect(mockDoubleSeriesSet[1].renderer.setSeriesState).toHaveBeenCalled();
            });
        });

        describe("destroy >", () => {
            it(`should unsubscribe the highlightDataPointSubscription`, () => {
                (renderEngine as any).highlightDataPointSubscription = new Subscription();
                spyOn((renderEngine as any).highlightDataPointSubscription, "unsubscribe");
                renderEngine.destroy();
                expect((renderEngine as any).highlightDataPointSubscription.unsubscribe).toHaveBeenCalled();
            });
        });
