...
export class RenderEngine {
    ...
    constructor(private lasagna: Lasagna, private dataManager: DataManager) {
        this.highlightDataPointSubscription = this.rendererSubject.subscribe((event: IRendererEventPayload) => {
            if (event.eventName === HIGHLIGHT_DATA_POINT_EVENT) {
                this.highlightDataPoint(event.data.seriesId, event.data.index);
            }
        });
    }
    ...
    public update(): void {
        ...
        each(this.dataManager.chartSeriesSet, (chartSeries: IChartSeries<IAccessors>) => {
            ...
            if (areDomainsValid) {
                chartSeries.renderer.draw({ dataSeries: chartSeries, containers: childContainers, scales: chartSeries.scales }, this.rendererSubject);
            }
        });
    }
    ...
    public emitInteractionDataPoints(payload: IInteractionValuesPayload): void {
        ...
        const interactionDataPoints = this.dataManager.chartSeriesSet.reduce((result, chartSeries: IChartSeries<IAccessors>) => {
            ...
            if (chartSeries.data && chartSeries.data.length > 0) {
                dataIndex = chartSeries.renderer.getDataPointIndex(chartSeries, chartValues, chartSeries.scales);
            }
            const position: IPosition | undefined = chartSeries.renderer.getDataPointPosition(chartSeries, dataIndex, chartSeries.scales);
            ...
            if (payload.interactionType === InteractionType.MouseMove) {
                const prevHighlight = this.highlightedDataPoints[chartSeries.id];
                if (!isUndefined(dataIndex) && (isUndefined(prevHighlight) ||
                    (dataIndex === DATA_POINT_INTERACTION_RESET || dataIndex !== prevHighlight.index))) {
        ...
        if (payload.interactionType !== InteractionType.MouseMove || highlightChangeDetected) {
            this.interactionDataPointsSubject.next({ interactionType: payload.interactionType, dataPoints: interactionDataPoints });
        }
    ...
    public highlightDataPoint(seriesId: string, index: number): void {
        ...
        chartSeries.renderer.highlightDataPoint(renderSeries, index, this.rendererSubject);
    }
    ...
    private buildLayerIndex(chartSeriesSet: IChartSeries<IAccessors>[]): { [layerName: string]: ILayerIndexEntry } {
        ...
        each(chartSeriesSet, chartSeries => {
            each(chartSeries.renderer.getRequiredLayers(), layerDef => {
                if (STANDARD_RENDER_LAYERS[layerDef.name] && !isEqual(STANDARD_RENDER_LAYERS[layerDef.name], layerDef)) {
                    throw new Error(`Custom definition detected for render layer "${layerDef.name}". ` +
    ...
    private addNeededLayers(): void {
        each(this.layerIndex, layerData => {
            if (!this.renderLayers[layerData.layerDefinition.name]) {
                this.renderLayers[layerData.layerDefinition.name] = this.lasagna.addLayer(layerData.layerDefinition);
            }
        });
    }
    ...
    private updateLayerContents(): void {
        each(Object.keys(this.renderLayers), layerName => {
            ...
            const containers = renderLayer.selectAll<SVGElement, IChartSeries<IAccessors>>("g.data-series-container")
                .data(this.layerIndex[layerName].seriesUsing, (d: IChartSeries<IAccessors>) => d.id)
                .order();

            containers.enter().append("g")
                .attrs({
                    "id": (d: IChartSeries<IAccessors>) => this.getChildContainerId(layerName, d.id),
    ...
    private getSeriesChildContainers(seriesId: string): IRenderContainers {
        ...
        each(Object.keys(this.renderLayers), layerName => {
            const renderLayer = this.renderLayers[layerName];
            childContainers[layerName] = renderLayer.selectAll(`#${layerName}-${UtilityService.cssEscape(seriesId)}`);
        });
