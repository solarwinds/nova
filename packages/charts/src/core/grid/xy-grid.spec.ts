...
describe("XYGrid >", () => {
    ...
    function buildAndUpdateAxes() {
        (<any>grid).buildAxes(
            grid.getLasagna().getLayerContainer(Grid.GRID_ELEMENTS_LAYER_NAME)
        );
        (<any>grid).updateAxes();
    ...
    describe("x-axis", () => {
        it("should have pointer events disabled", () => {
            ...
            expect(window.getComputedStyle(axisLeftNode).pointerEvents).toEqual(
                "none"
            );
        });
    });
    ...
    describe("y-axis", () => {
        it("left axis should have pointer events disabled", () => {
            const axisLeftNode = (
                chart.getGrid() as any
            ).axisYLeft.group.node();
            expect(window.getComputedStyle(axisLeftNode).pointerEvents).toEqual(
                "none"
            );
        });
    ...
    describe("update", () => {
        ...
        it("should invoke updateRanges if the left margin changes", fakeAsync(() => {
            ...
            spyOn(<any>grid, "getMaxTextWidth").and.returnValue(
        ...
        it("should invoke updateRanges if the right margin changes", fakeAsync(() => {
            ...
            spyOn(<any>grid, "getMaxTextWidth").and.returnValue(
                gridConfig.dimension.margin.right + 5
            );
            const spy = spyOn(grid, "updateRanges");
        ...
        it("should invoke next on the updateChartDimensionsSubject only once if the right margin changes on repeated update calls", fakeAsync(() => {
            ...
            const updateChartDimensionsSpy = spyOn(
                grid.updateChartDimensionsSubject as Subject<void>,
                "next"
            );

            // mock a change in the right margin
            getMaxTextWidthSpy.and.returnValue(
                gridConfig.dimension.margin.right + 5
            );
    ...
    describe("adjustAxisTicks", () => {
        it("should set the cursor to 'default'", fakeAsync(() => {
            ...
            domain(grid.scales["x"].index[grid.bottomScaleId as string], [
                "Q1",
                "Q2",
                "Q3",
            ]);
            ...
            (<any>grid).adjustAxisTicks(
                (<any>grid).axisX.labelGroup,
                grid.scales["x"].index[grid.bottomScaleId as string]
            );

            expect((<any>grid).axisX.labelGroup.attr("cursor")).toEqual(
                "default"
            );
        ...
        it("should set pointer-events to true on each label", fakeAsync(() => {
            ...
            const allAxisLabels = (<any>grid).selectAllAxisLabels(
                (<any>grid).axisX.labelGroup
            );
        ...
        it("should add the IGNORE_INTERACTION_CLASS to the text element of each label", fakeAsync(() => {
            ...
            allAxisLabels.forEach((group: HTMLElement) => {
                ...
                groupSelection
                    .select("text")
                    .classed(IGNORE_INTERACTION_CLASS, false);
            });
            ...
            allAxisLabels.forEach((group: HTMLElement) => {
                ...
                expect(
                    groupSelection
                        .select("text")
                        .classed(IGNORE_INTERACTION_CLASS)
                ).toEqual(true);
            });
        ...
        it("should append a title to each text element", fakeAsync(() => {
            ...
            const allAxisLabels = (<any>grid).selectAllAxisLabels(
                (<any>grid).axisYLeft.labelGroup
            );
            (<any>grid).adjustAxisTicks((<any>grid).axisX.labelGroup, scale);
            allAxisLabels.forEach((group: HTMLElement) => {
                const groupSelection = select(group);
                expect(groupSelection.select("title").text()).toEqual(
                    `${groupSelection.data()[0]}`
                );
            });
    ...
    describe("handleTickLabelOverflow", () => {
        describe("pointer-events class", () => {
            it("should be absent during the debounce interval", fakeAsync(() => {
                grid.scales["x"] = createScale(
                    new BandScale(testBottomScaleId)
                );
                domain(grid.scales["x"].index[grid.bottomScaleId as string], [
                    "Q1",
                    "Q2",
                ...
                (<any>grid).handleTickLabelOverflow(
                    (<any>grid).axisX.labelGroup,
                    grid.scales["x"].index[grid.bottomScaleId as string],
                    allAxisLabels
                );
                allAxisLabels.forEach((group: HTMLElement) => {
                    const groupSelection = select(group);
                    expect(groupSelection.classed("pointer-events")).toEqual(
                        false
                    );
                });
        ...
        describe("tick-hidden-text class", () => {
            it("should be present during the debounce interval", fakeAsync(() => {
                ...
                expect(
                    (<any>grid).axisX.labelGroup.classed("tick-hidden-text")
                ).toEqual(true);
        ...
        describe("for the bottom axis", () => {
            it("should not invoke the 'overflowHandler' if the scale is continuous", fakeAsync(() => {
                grid.scales["x"] = createScale(
                    new LinearScale(testBottomScaleId)
                );
                buildAndUpdateAxes();
                ...
                const spy = spyOn(
                    config.axis.bottom.tickLabel,
                    "overflowHandler" as never
                );
                (<any>grid).handleTickLabelOverflow(
                    (<any>grid).axisX.labelGroup,
                    grid.scales["x"].index[grid.bottomScaleId as string],
            ...
            it("should invoke the 'overflowHandler' if the scale is not continuous", fakeAsync(() => {
                ...
                if (
                    (<any>grid).axisX.labelGroup
                        .select(".sample-ellipsis")
                        .empty()
                ) {
                    const testText = (<any>grid).axisX.labelGroup.append(
                        "text"
                    );
                ...
                const widthLimit = (
                    grid.scales["x"].index[
                        grid.bottomScaleId as string
                    ] as BandScale
                ).bandwidth();
                const horizontalPadding =
                    config.axis.bottom.tickLabel.horizontalPadding;
                ...
                expect(
                    config.axis.bottom.tickLabel.overflowHandler
                ).toHaveBeenCalledWith(jasmine.anything() as never, {
                    widthLimit,
                    horizontalPadding,
                    ellipsisWidth,
                });
        ...
        describe("for the left axis", () => {
            it("should invoke the 'overflowHandler' if 'fit' is false", fakeAsync(() => {
                config.axis.left.fit = false;
                domain(
                    grid.scales["y"].index[grid.leftScaleId as string],
                    [0, 100]
                );
                buildAndUpdateAxes();
                ...
                if (
                    ...
                    const testText = (<any>grid).axisYLeft.labelGroup.append(
                        "text"
                    );
                    const ellipsis = testText
                        .classed("sample-ellipsis", true)
                ...
                const widthLimit =
                    config.dimension.margin.left -
                    config.axis.left.padding -
                    config.axis.left.tickSize;
                const horizontalPadding =
                    config.axis.left.tickLabel.horizontalPadding;
                ...
                // using jasmine.anything() as a stand-in for the text element
                expect(
                    config.axis.left.tickLabel.overflowHandler
                ).toHaveBeenCalledWith(jasmine.anything() as never, {
                    widthLimit,
                    horizontalPadding,
                    ellipsisWidth,
        ...
        describe("for the right axis", () => {
            it("should invoke the 'overflowHandler' if 'fit' is false", fakeAsync(() => {
                ...
                domain(
                    grid.scales["y"].index[grid.rightScaleId as string],
                    [0, 100]
                );
                buildAndUpdateAxes();

                const allAxisLabels = (<any>grid).selectAllAxisLabels(
                    (<any>grid).axisYRight.labelGroup
                );
                let ellipsisWidth = 0;
                if (
                    (<any>grid).axisYRight.labelGroup
                    ...
                    const testText = (<any>grid).axisYRight.labelGroup.append(
                        "text"
                    );
                    const ellipsis = testText
                        .classed("sample-ellipsis", true)
                ...
                const widthLimit =
                    config.dimension.margin.right -
                    config.axis.right.padding -
                    config.axis.right.tickSize;
                const horizontalPadding =
                    config.axis.right.tickLabel.horizontalPadding;
                ...
                // using jasmine.anything() as a stand-in for the text element
                expect(
                    config.axis.right.tickLabel.overflowHandler
                ).toHaveBeenCalledWith(jasmine.anything() as never, {
                    widthLimit,
                    horizontalPadding,
                    ellipsisWidth,
    ...
    describe("fitBottomAxis", () => {
        it("should not change the right margin if the label for the last tick on the bottom axis doesn't exist", () => {
            ...
            spyOn((<any>grid).axisX.labelGroup, "select").and.returnValue(
                mockTextSelection
            );

            (<any>grid).fitBottomAxis(grid.config().dimension);

            expect(grid.config().dimension.margin.right).toEqual(
                expectedRightMargin
            );
        });
        ...
        it("should change the right margin based on the width of the right-most tick label on the bottom axis", () => {
            ...
            const expectedRightMargin =
                mockTextSelection.node().getBoundingClientRect().width / 2 -
                Math.abs(d3Scale.range()[1] - d3Scale()) / 2;
            grid.config().dimension.margin.right = expectedRightMargin + 1;
            grid.scales = {
                x: { list: [{ d3Scale }] },
            } as unknown as ScalesIndex;
            spyOn((<any>grid).axisX.labelGroup, "select").and.returnValue(
                mockTextSelection
            );
    ...
    describe("scales", () => {
        it("should set the scales domain calculator to getAutomaticDomainWithTicks if it is linear", () => {
            ...
            expect(
                grid?.scales?.x?.list[0]?.__domainCalculatedWithTicks
            ).toBeTruthy();
            expect(
                grid?.scales?.x?.list[1]?.__domainCalculatedWithTicks
            ).toBeFalsy();
            expect(
                grid?.scales?.y?.list[0]?.__domainCalculatedWithTicks
            ).toBeTruthy();
        });
        ...
        it("should wrap the the scales domain calculator to getAutomaticDomainWithTicks if the domain calculator is not getAutomaticDomain", () => {
            ...
            expect(isDomainWithTicksCalculator(yScale.domainCalculator)).toBe(
                true
            );
        });
    });
