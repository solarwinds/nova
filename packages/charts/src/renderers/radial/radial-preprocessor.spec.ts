...
describe("Radial Preprocessor >", () => {
    ...
    beforeEach(() => {
        ...
        generateSeriesSet = (donutSeriesSet: any): any[] => donutSeriesSet.map((dataSeries: any) => ({
            ...dataSeries,
            scales: {
                r: scale,
            },
    ...
    it("should modify generated data", () => {
        const res = radialPreprocessor(generateSeriesSet(generatePieData(["Up", "Down"])), () => true);
    ...
    it("should modify pie value, but not series data when not visible", () => {
        const res = radialPreprocessor(generateSeriesSet(generatePieData(["Up", "Down"])), (series: IChartSeries<IAccessors>) => series.name === "Up");
        expect(res[0].data[0].value).toBe(10);
