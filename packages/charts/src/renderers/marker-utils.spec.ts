...
import { DATA_POINT_INTERACTION_RESET, INTERACTION_DATA_POINT_EVENT } from "src/constants";
...
describe("Marker Utils", () => {
    ...
    describe("manageMarker", () => {
        ...
        it("creates a marker when it doesn't exist", () => {
            const drawMarkerSpy = spyOn(MarkerUtils, "drawMarker").and.callThrough();

            MarkerUtils.manageMarker(dataSeries, scales, 0, containers[RenderLayerName.foreground], new Subject<IRendererEventPayload>());
        ...
        it("removes a marker when it doesn't exist", () => {
            const drawMarkerSpy = spyOn(MarkerUtils, "drawMarker").and.callThrough();
            const removeMarkerSpy = spyOn(MarkerUtils, "removeMarker").and.callThrough();
            ...
            MarkerUtils.manageMarker(dataSeries, scales, 1, containers[RenderLayerName.foreground], new Subject<IRendererEventPayload>());
    ...
    describe("drawMarker", () => {
        ...
        let dataPointSubject: Subject<IRendererEventPayload<IInteractionDataPointEvent>>;
        let dataPoint: IDataPoint;

        beforeEach(() => {
            const x = scales.x.convert(dataSeries.accessors.data.x(dataSeries.data[1], 1, dataSeries.data, dataSeries));
            const y = scales.y.convert(dataSeries.accessors.data.y(dataSeries.data[1], 1, dataSeries.data, dataSeries));
            dataPoint = {
                index: 1,
            ...
            dataPointSubject = new Subject<IRendererEventPayload<IInteractionDataPointEvent>>();
        });
