...
export function applyStatusEndpoints(timeframeFilter: ITimeframe,
                                     ...
                                     originalSeriesData: ITimeseriesWidgetData[]): ITimeseriesWidgetData[] {
    ...
    originalSeriesData.forEach((series: ITimeseriesWidgetData, i: number) => {
        ...
        if (filteredData.length > 0) {
            ...
            startIndex = series.data.findIndex(datum => moment(datum.x).isSame(filteredData[0].x));
            endIndex = series.data.findIndex(datum => moment(datum.x).isSame(filteredData[filteredData.length - 1].x));
        } else {
            ...
            startIndex = series.data.findIndex(datum => moment(datum.x).isAfter(timeframeFilter.startDatetime));
            endIndex = series.data.findIndex(datum => moment(datum.x).isAfter(timeframeFilter.endDatetime)) - 1;
        }

        const filterStartMoment = moment(timeframeFilter.startDatetime);
        if ((filteredData.length === 0 || !filterStartMoment.isSame(filteredData[0].x)) &&
            filterStartMoment.isBetween(series.data[0].x, series.data[series.data.length - 1].x)) {
            filteredDataWithEndpoints[i].data.unshift({ ...series.data[startIndex - 1], x: timeframeFilter.startDatetime.toDate() });
        }

        const filterEndMoment = moment(timeframeFilter.endDatetime);
        if (((filteredData.length === 0 || !filterEndMoment.isSame(filteredData[filteredData.length - 1].x)) &&
            filterEndMoment.isBetween(series.data[0].x, series.data[series.data.length - 1].x))) {
            filteredDataWithEndpoints[i].data.push({ ...series.data[endIndex], x: timeframeFilter.endDatetime.toDate() });
        }
    });
