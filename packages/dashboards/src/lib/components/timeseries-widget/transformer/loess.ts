...
export function loess() {
  ...
  function smooth(xval: number[], yval: number[], weights: number[]) {
    ...
    if (n !== yval.length) {throw new Error("Mismatched array lengths");}
    if (n === 0) {throw new Error("At least one point required.");}
    ...
    if (bandwidthInPoints < 2) {throw new Error("Bandwidth too small.");}
    ...
    while (++iter <= robustnessIters) {
      ...
      i = -1; while (++i < n) {
        ...
        const beta = (Math.sqrt(Math.abs(meanXSquared - meanX * meanX)) < accuracy)
...
function science_stats_loessUpdateBandwidthInterval(
  xval: number[], weights: number[], i: number, bandwidthInterval: number[]) {
