...
export class WidgetConfiguratorSectionHeaderPipe implements PipeTransform {
    ...
    transform(headerText: string | undefined, index: number, headerPrefix: string = this.defaultHeaderPrefix): string {
        return `${headerPrefix} ${index + 1}${headerText ? " - " : ""}${headerText || ""}`;
    }
}
