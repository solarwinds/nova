...
export class TimeseriesSeriesConverterService extends BaseConverter implements AfterViewInit, OnDestroy {
    ...
    constructor(@Inject(PIZZAGNA_EVENT_BUS) eventBus: EventBus<IEvent>,
        ...
        this.eventBus.subscribeUntil(DATA_SOURCE_CHANGE, this.destroy$, (v) => {
            ...
            const nodes = get(this.pizzagnaService.pizzagna, nodesPath) as unknown as string[];
            ...
            updatedPizzagna = immutableSet(updatedPizzagna, `${this.formSeriesComponentPath}.series`, []);
        ...
        this.eventBus.subscribeUntil(DATA_SOURCE_OUTPUT, this.destroy$, (event: IEvent<any | IDataSourceOutput<any>>) => {
            ...
            const { series } = isUndefined(event.payload.result) ? event.payload : (event.payload.result || {});

            if (series) {
                this.dataSourceSeries$.next(series);

                let updatedPizzagna = this.pizzagnaService.pizzagna;
                updatedPizzagna = immutableSet(updatedPizzagna, `${this.formSeriesComponentPath}.allSeries`, series);

                const seriesTemplate = this.pizzagnaService.pizzagna[PizzagnaLayer.Structure][this.component.componentId].properties?.template;
                const indicatorDataIndex = seriesTemplate.findIndex((val: IItemConfiguration) => val.id === this.indicatorDataKey);
                const indicatorDataPath = `${PizzagnaLayer.Structure}.series.properties.template[${indicatorDataIndex}].properties.allSeries`;
                updatedPizzagna = immutableSet(updatedPizzagna, indicatorDataPath, series);

                const previewSeries = get(this.getPreview(), `chart.${this.previewSeriesPath}`, []) as ITimeseriesWidgetSeries[];
                updatedPizzagna = previewSeries.reduce((res, s) => immutableSet(
                    res,
                    `${PizzagnaLayer.Data}.${s.id}/${this.indicatorDataKey}.properties.allSeries`,
                    series), updatedPizzagna);
                ...
                const editorSeries = get(this.pizzagnaService.pizzagna, editorSeriesPath);
                if (isEmpty(editorSeries)) {
                    this.shouldReadForm = false;

                    const seriesForPreview: ITimeseriesWidgetSeries[] = series.map((s: ITimeseriesWidgetData) => ({
                        id: s.id,
                        selectedSeriesId: s.id,
                        label: s.name,
                    }));
                    const updatedPreview = immutableSet(this.getPreview(), `chart.${this.previewSeriesPath}`, seriesForPreview);
    ...
    public buildForm(): void {
        ...
        const series = get(preview, `chart.${this.previewSeriesPath}`, []) as ITimeseriesWidgetSeries[];
        ...
        const pizzagnaWithSeries = immutableSet(editorPizzagna, `${this.formSeriesComponentPath}.series`, seriesToSet);
    ...
    public toPreview(form: FormGroup): void {
        form.valueChanges
            ...
            .subscribe(formSeries => {
                if (!this.shouldReadForm) { return; }
                const preview = this.getPreview();
                const { chart } = preview;
                const seriesFromPreview = get(chart, this.previewSeriesPath, []) as ITimeseriesWidgetSeries[];

                const series: ITimeseriesWidgetSeries[] = formSeries.map((s: ITimeseriesWidgetSeries) => ({
                    id: s.id,
                    selectedSeriesId: s.selectedSeriesId,
                    ...seriesFromPreview.find(previewSeries => previewSeries.id === s.id),
                }));

                const updatedPreview = immutableSet(preview, `chart.${this.previewSeriesPath}`, series);
                this.updatePreview(updatedPreview);

                const selectedSeriesIds = formSeries.map((s: ITimeseriesWidgetSeries) => get(s, `properties[${s.id}/indicatorData].id`))
    ...
    private updateAvailableSeries(availableSeries: ITimeseriesWidgetData[]) {
        ...
        updatedPizzagna = immutableSet(updatedPizzagna, `${this.formSeriesComponentPath}.availableSeries`, availableSeries);

        // template
        const seriesTemplate = this.pizzagnaService.pizzagna[PizzagnaLayer.Structure][this.component.componentId].properties?.template;
        const indicatorDataIndex = seriesTemplate.findIndex((val: ITimeseriesWidgetSeries) => val.id === this.indicatorDataKey);
        const indicatorDataPath = `${PizzagnaLayer.Structure}.series.properties.template[${indicatorDataIndex}].properties.availableSeries`;
        updatedPizzagna = immutableSet(updatedPizzagna, indicatorDataPath, availableSeries);

        // already created forms
        const seriesIds = get(this.pizzagnaService.pizzagna, `${PizzagnaLayer.Structure}.series.properties.nodes`) as unknown as string[];
        updatedPizzagna = seriesIds.reduce((res, id) => immutableSet(
            res,
    ...
    private subscribeToAvailableDataFieldsChange() {
        combineLatest([this.dataSourceSeries$, this.selectedSeriesIds$])
            .pipe(
                ...
                map(([dsSeries, usedSeriesIds]) => dsSeries.filter(ds => !usedSeriesIds.find(id => id === ds.id))),
