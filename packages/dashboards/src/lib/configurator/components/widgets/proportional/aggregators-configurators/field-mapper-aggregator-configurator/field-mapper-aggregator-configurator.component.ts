...
import { EventBus, IDataField, IDataSource, IEvent, LoggerService } from "@solarwinds/nova-bits";
...
export class FieldMapperAggregatorConfiguratorComponent extends AggregatorMetricSelectorConfigurationComponent
    ...
    public chartSeriesDataFields: IDataField[] = [DEFAULT_CHART_SERIES_DATA_FIELD];
    ...
    constructor(changeDetector: ChangeDetectorRef,
        ...
        this.eventBus.getStream(DATA_SOURCE_CREATED)
            ...
            .subscribe((event: IEvent<IDataSource>) => {
                ...
                if (dataSource) {
                    (<IProportionalDataFieldsConfig>dataSource.dataFieldsConfig)?.chartSeriesDataFields$
                        ?.pipe(takeUntil(this.destroy$))
                        .subscribe((dataFields) => this.chartSeriesDataFields = dataFields);
                }
            });
    }
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if (changes.chartSeriesDataFieldId) {
            this.form.get("chartSeriesDataFieldId")?.setValue(changes.chartSeriesDataFieldId.currentValue);
        }
    }
