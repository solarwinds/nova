...
export class ToastAtom extends Atom {
    ...
    private content = this.root.all(by.className(`${ToastAtom.CSS_CLASS}__content`)).first();
    public dismiss: ButtonAtom = Atom.findIn(ButtonAtom, this.content.all(by.className(`${ToastAtom.CSS_CLASS}__dismiss-button`)).first());


    public getTitle = async (): Promise<string> => this.root.element(by.css(`.${ToastAtom.CSS_CLASS}__content-title`)).getText();

    public getBody = async (): Promise<string> => this.getBodyElement().getText();

    public isSuccessType = async (): Promise<boolean> => this.isToastType("success");

    public isWarningType = async (): Promise<boolean> => this.isToastType("warning");
    ...
    public waitUntilNotDisplayed = async (timeOut: number = ToastAtom.toastTimeout): Promise<void> =>
        browser.wait(ExpectedConditions.stalenessOf(this.root), timeOut)

    public getToastsContainerPositioning = async (positionClass: string): Promise<boolean> =>
        Atom.hasClass(this.container, positionClass)

    public click = async (): Promise<void> => this.root.click();

    public unhover = async (): Promise<void> => browser.actions().mouseMove(this.container, { x: -1, y: -1 }).perform();
