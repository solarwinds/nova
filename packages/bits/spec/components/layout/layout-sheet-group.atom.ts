...
export class LayoutSheetGroupAtom extends Atom {
    ...
    public async mouseDownHorizontalResizerByIndex(index: number): Promise<void> {
        await browser.actions().mouseMove(this.getHorizontalResizerByIndex(index)).perform();
        await browser.actions().mouseDown().perform();
    }

    public async mouseDownVerticalResizerByIndex(index: number): Promise<void> {
        await browser.actions().mouseMove(this.getVerticalResizerByIndex(index)).perform();
        await browser.actions().mouseDown().perform();
    }
    ...
    public async moveLeftHorizontalResizerByIndex(index: number, pixels: number): Promise<void> {
        await this.mouseDownHorizontalResizerByIndex(index);
        await browser.actions().mouseMove({ x: -pixels, y: 0 }).perform();
        await this.mouseUp();
    }

    public async moveRightHorizontalResizerByIndex(index: number, pixels: number): Promise<void> {
        await this.mouseDownHorizontalResizerByIndex(index);
        await browser.actions().mouseMove({ x: pixels, y: 0 }).perform();
        await this.mouseUp();
    }

    public async moveUpVerticalResizerByIndex(index: number, pixels: number): Promise<void> {
        await this.mouseDownVerticalResizerByIndex(index);
        await browser.actions().mouseMove({ x: 0, y: -pixels }).perform();
        await this.mouseUp();
    }

    public async moveDownVerticalResizerByIndex(index: number, pixels: number): Promise<void> {
        await this.mouseDownVerticalResizerByIndex(index);
        await browser.actions().mouseMove({ x: 0, y: pixels }).perform();
        await this.mouseUp();
    }
