...
export class ExpanderAtom extends Atom {
    ...
    public isCollapsed = async (): Promise<boolean> => !(await this.isExpanded());

    public async isContentDisplayed(cssSelectorSet: string | string[]): Promise<boolean> {
    ...
    public async isContentAttachedToDOM(cssSelectorSet: string | string[]): Promise<boolean> {
        if (!Array.isArray(cssSelectorSet)) {
            cssSelectorSet = [cssSelectorSet];
        }
    ...
    public isHeaderIconPresent = async (): Promise<boolean> =>
        IconAtom.findIn(IconAtom, this.getElement().element(by.css(".nui-expander__header-content-icon"))).getElement().isPresent()
    ...
    public getHeadingText = async (): Promise<string> =>
        this.root.element(by.css(".nui-expander__header-content-wrapper")).getText()

    public getHeaderHeight = async (): Promise<number> =>
        (await this.root.element(by.css(".nui-expander__header-title")).getSize()).height

    public getCustomHeaderWidth = async (): Promise<number> =>
        (await this.root.element(by.css(".nui-expander__custom-header")).getSize()).width
}
