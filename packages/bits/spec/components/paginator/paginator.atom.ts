...
export class PaginatorAtom extends Atom {
    ...
    public async pageLinkClick(pageNumber: number): Promise<void> {
        return super.getElement().all(by.css(".nui-paginator__list li")).get(pageNumber).click();
    }
    ...
    public async ellipsedPageLinkClick(pageNumber: number): Promise<void> {
        const allIttems: any = await super.getElement().all(by.className("nui-paginator__page-cell")).getText();
        const index = allIttems.findIndex((itemText: string) => itemText === pageNumber.toString());
        return super.getElement().all(by.className("nui-paginator__page-cell")).get(index).click();
    }

    public ellipsisLink(index: number): ElementFinder { return super.getElement().all(by.className("nui-paginator__dots")).get(index); }

    public ellipsisLinkClick = async (index: number): Promise<void> => this.ellipsisLink(index).click();
    ...
    public prevLink(): ElementFinder { return super.getElement().all(by.className(this.prevNextClass)).get(0); }

    public nextLink(): ElementFinder { return super.getElement().all(by.className(this.prevNextClass)).get(1); }
    ...
    public async pageCount(): Promise<number> {
        return super.getElement().all(by.css(".nui-paginator__list li")).getAttribute("value").then(async (arrVal: any) => {
            const resultArr: number[] = await arrVal.map((value: string) => parseInt(value, 10));
    ...
    public async ellipsisHasTopClass(): Promise<boolean> {
        ...
        const dropdownElement = super.getElement().all(by.css(".nui-popup__area")).get(0);

        return await Atom.hasClass(dropdownElement, "nui-popup__area--top");
    }

    public async itemsDispHasTopClass(): Promise<boolean> {
        const itemsShownElem = super.getElement().all(by.css(".nui-paginator__items-shown")).get(0);
    ...
    public getPageNumberButton(pageNumber: string): ElementFinder {
        return this.getElement().element(by.cssContainingText("button.nui-button", pageNumber));
    }
}
