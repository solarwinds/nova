...
describe("USERCONTROL popover", () => {
    ...
    const openProgrammaticallyButtonId = "nui-demo-popover-open-programmatically-btn";
    const closeProgrammaticallyButtonId = "nui-demo-popover-close-programmatically-btn";
    const popoverOpenCloseProgrammaticallyId = "nui-demo-popover-open-close-programmatically";
    ...
    beforeAll(() => {
        ...
        popoverMouseEnterTrigger.popoverModalId = "nui-demo-popover-modal-mouseenter";
        ...
        popoverBottomPlacement = Atom.find(PopoverAtom, bottomPlacementParentId);
        ...
        containerPopover = element(by.css("#nui-demo-popover-container .nui-popover-container"));
        popoverOpenCloseProgrammatically = Atom.find(PopoverAtom, popoverOpenCloseProgrammaticallyId);
        ...
        openProgrammaticallyButton = Atom.find(ButtonAtom, openProgrammaticallyButtonId);
        closeProgrammaticallyButton = Atom.find(ButtonAtom, closeProgrammaticallyButtonId);
    });
    ...
    describe("title", () => {
        ...
        it("should show title when is defined and component is clicked", async () => {
            ...
            await expect(await popoverClickTrigger.getTitleText()).toEqual("Popover with Click Trigger");
        });

        it("should not close popover after click on itself", async () => {
            await popoverPreventClosing.clickTarget();
            await expect(await popoverPreventClosing.isPopoverDisplayed()).toBe(true);
    ...
    describe("triggers", () => {
        it("should honor 'click' trigger", async () => {
            await expect(await popoverClickTrigger.isPopoverDisplayed()).toBe(false);
            await popoverClickTrigger.togglePopover();
            await expect(await popoverClickTrigger.isPopoverDisplayed()).toBe(true);
            await popoverClickTrigger.togglePopover();
            await expect(await popoverClickTrigger.isPopoverDisplayed()).toBe(false);
        ...
        it("should honor 'focus' trigger", async () => {
            ...
            await expect(await popoverFocusTrigger.isPopoverDisplayed()).toBe(false);
        });

        describe("should honor 'mouseenter' trigger as default trigger", () => {
            it("without any delays", async () => {
                await expect(await popoverMouseEnterTrigger.isPopoverDisplayed()).toBe(false);
                await popoverMouseEnterTrigger.openByHover();
                await expect(await popoverMouseEnterTrigger.isPopoverDisplayed()).toBe(true);
            ...
            it("with a " + delay + "ms delay", async () => {
                await expect(await popoverDebounce1.isPopoverDisplayed()).toBe(false);
                ...
                await expect(await popoverDebounce1.isPopoverDisplayed()).toBe(true);

                const popoverUntilDisplayDuration = (new Date()).getTime() - hoverStartTime;
                expect(popoverUntilDisplayDuration).toBeGreaterThanOrEqual(delay);
            });
    ...
    describe("placement", () => {
        it("should honor left 'placement'", async () => {
            ...
            await expect(await popoverLeftPlacement.isDisplayedLeft()).toBe(true);
            await expect(await popoverLeftPlacement.isDisplayedRight()).toBe(false);
        });
        it("should honor right 'placement'", async () => {
            await popoverRightPlacement.open();
            await expect(await popoverRightPlacement.isDisplayedLeft()).toBe(false);
            await expect(await popoverRightPlacement.isDisplayedRight()).toBe(true);
        });
        ...
        xit("appear on the left to adjust for the lack of free space on the right", async () => {
            ...
            await popoverMouseEnterTrigger.openByHover();
            await expect(await popoverMouseEnterTrigger.isDisplayedRight()).toBe(false);
            await expect(await popoverMouseEnterTrigger.isDisplayedLeft()).toBe(true);
        });
        ...
        it("should close on user action", async () => {
            ...
            await expect(await popoverPreventClosing.isPopoverDisplayed()).toBe(true);
            const userPopoverCloseButton = Atom.find(ButtonAtom, "nui-demo-custom-close-popover");

            await userPopoverCloseButton.click();
            await popoverPreventClosing.waitForClosed();
            await expect(await popoverPreventClosing.isPopoverDisplayed()).toBe(false);
        });
        ...
        describe("vertical placement", () => {
            it("should open popover at the bottom", async () => {
                ...
                expect(await popoverBottomPlacement.isDisplayedBottom()).toBe(true);
            });
    ...
    describe("opening and closing programmatically >", () => {
        it("should open popover by clicking on button", async () => {
            ...
            await expect(await popoverOpenCloseProgrammatically.isPopoverDisplayed()).toBe(true);
        });
        ...
        it("should close popover by clicking on button", async () => {
            ...
            await expect(await popoverOpenCloseProgrammatically.isPopoverDisplayed()).toBe(false);
        });
    });
});
