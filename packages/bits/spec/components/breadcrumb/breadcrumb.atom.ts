...
export class BreadcrumbAtom extends Atom {
    ...
    public getAllItems(): ElementArrayFinder { return super.getElement().all(by.css(".nui-breadcrumb__wrapper")); }

    public getItemsCount = async (): Promise<number> => this.getAllItems().count();

    public getLastItem = (): ElementFinder =>  this.getAllItems().last();

    public getFirstItem = (): ElementFinder => this.getAllItems().first();

    public getLink = (item: ElementFinder): ElementFinder => item.element(by.className("nui-breadcrumb__crumb"));
