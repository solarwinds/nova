...
export class SelectorAtom extends Atom {
    ...
    public async selectAppendedToBodyItem(selectionType: SelectionType): Promise<void> {
        if (!(await this.getPopupAtom().isOpenedAppendToBody())) {
            await this.getToggle().click();
        }
        const menuPopup = Atom.findIn(MenuPopupAtom, this.getElement().element(by.className("nui-menu-popup"))).getElement();
    ...
    public getSelectorButton(): ButtonAtom {
        const buttonContainer = super.getElement().element(by.className("nui-selector__checkbox-button"));
