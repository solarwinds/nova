...
export class BasicSelectAtom extends Atom {
    ...
    public getElementByClass = (className: string): ElementFinder => this.getElement().element(by.className(className));

    protected getElementByTagName = (tagName: string): ElementFinder => this.getElement().element(by.tagName(tagName));

    protected getElementByCss = (selector: string): ElementFinder => this.getElement().element(by.css(selector));
