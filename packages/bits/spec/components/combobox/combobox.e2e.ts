...
xdescribe("USERCONTROL Combobox >", () => {
    ...
    beforeAll(() => {
        ...
        comboboxDisabled = Atom.find(ComboboxAtom, "nui-demo-combobox-disabled");
        inlineCombobox = Atom.find(ComboboxAtom, "nui-demo-inline-combobox");
        comboboxJustified = Atom.find(ComboboxAtom, "nui-demo-justified-combobox");
        comboboxRequired = Atom.find(ComboboxAtom, "nui-demo-combobox-required");
        comboClearOnBlur = Atom.find(ComboboxAtom, "nui-demo-combobox-with-clear-on-blur");
        comboboxWithIcon = Atom.find(ComboboxAtom, "nui-demo-combobox-with-icon");
        reactiveFormCombobox = Atom.find(ComboboxAtom, "nui-demo-combobox-reactive-form");
        comboboxWithDisplayValue = Atom.find(ComboboxAtom, "nui-demo-display-value-combobox");
        comboboxTypeahead = Atom.find(ComboboxAtom, "nui-demo-combobox-typeahead");
        comboboxSeparators = Atom.find(ComboboxAtom, "nui-demo-combobox-separators");
        comboboxHTMLItems = Atom.find(ComboboxAtom, "nui-demo-combobox-html-in-items");
        comboboxWithTemplate = Atom.find(ComboboxAtom, "nui-demo-combobox-with-template");
        comboboxAppendToBody = Atom.find(ComboboxAtom, "nui-demo-combobox-append-to-body");
        comboboxRemoveValueButton = Atom.find(ComboboxAtom, "nui-demo-combobox-remove-value-button");
    });
    ...
    describe("combobox >", () => {
        describe("basic >", () => {
            it("should display placeholder", async () => {
                ...
                expect(await comboboxBasic.getComboboxPlaceholder()).toEqual(placeholder);
            });
            ...
            describe("when a value is picked from combobox, it", () => {
                ...
                it("should mark the selected item in the combobox menu", async () => {
                    ...
                    expect((await selectedItem.getAttribute("innerText")).trim()).toEqual(target);
                });
            });
            ...
            describe("on blur event", () => {
                it("should save new value on blur event if input value has changed", async () => {
                    ...
                    const selectedItem = comboboxBasic.getSelectedItem();
                    expect((await selectedItem.getAttribute("innerText")).trim()).toEqual(newValue);
                });
            });
        });
        ...
        describe("disabled >", () => {
            it("should have disabled styling", async () => {
                expect(await comboboxDisabled.getInput().isEnabled()).toEqual(false);
            });
        });
        ...
        describe("clear on blur >", () => {
            it("should clear input on blur if it's value is not in source array", async () => {
                ...
                await comboboxBasic.toggleMenu();
                expect(await comboClearOnBlur.getSelectedItems().count()).toEqual(0);
            });
        ...
        describe("justified >", () => {
            ...
            beforeEach(() => {
                widthInput = Atom.find(TextboxNumberAtom, "nui-test-width-input");
                component = comboboxJustified.getElementByClass("nui-combobox__layout-block");
                parent = browser.element(by.id("nui-demo-justified-combobox-container"));
            });
            ...
            async function compareWidths(expectedValue: number) {
                ...
                const containerLeftPadding = parseFloat(await parent.getCssValue("padding-left"));
                const containerRightPadding = parseFloat(await parent.getCssValue("padding-right"));
                const containerWidth = containerOuterWidth - containerLeftPadding - containerRightPadding;
        ...
        describe("inline >", () => {
            it("should have proper styles", async () => {
                const comboboxContainerDisplayStyle = await inlineCombobox.getLayoutBlock().getCssValue("display");
        ...
        describe("display value >", () => {
            it("should show correct values in input and dropdown", async () => {
                ...
                expect(await comboboxWithDisplayValue.getInputValue()).toEqual("Item 1");
            });
        });
        ...
        describe("reactive form >", () => {
            ...
            it("should set value both in textbox and menu when item is set from reactive form", async () => {
                ...
                expect(await reactiveFormCombobox.getSelectedItems().count()).toEqual(1);
                expect(await reactiveFormCombobox.isRequiredStyleDisplayed()).toBe(false);
        ...
        describe("typeahead, displayValue, clearOnBlur >", () => {
            ...
            it("should clear value if it's not in source array and re-render dropdown", async () => {
                ...
                expect(await comboboxTypeahead.getSelectedItems().count()).toEqual(0);
                await comboboxTypeahead.toggleMenu();
                expect(await comboboxTypeahead.getItemsCount()).toEqual(9);
            });
        });
        ...
        describe("with HTML-like strings, passed in source array", () => {
            it("should display HTML-like strings as strings and should not render them as DOM elements", async () => {
                ...
                expect(await comboboxHTMLItems.getItemText(1)).toEqual("<button>Button 1</button>");
                await comboboxHTMLItems.toggleMenu();
                await comboboxHTMLItems.select("<button>Button 1</button>");
                expect(await comboboxHTMLItems.getInputValue()).toEqual("<button>Button 1</button>");
            });
        });
        ...
        describe("focus >", () => {
            it("should be focused via keyboard", async () => {
                ...
                const activeElementClass = await (await browser.switchTo().activeElement()).getAttribute("class");
                expect(activeElementClass).toBe(inputClass);
            });

            it("should be focused via js", async () => {
                await browser.executeScript("arguments[0].focus()", await comboboxBasic.getInput().getWebElement());
                const inputClass = await comboboxBasic.getInput().getAttribute("class");
                const activeElementClass = await (await browser.switchTo().activeElement()).getAttribute("class");
                expect(activeElementClass).toBe(inputClass);
            });
        ...
        describe("custom template >", () => {
            it("applies the correct template", async () => {
                ...
                const getItemsWithNestedClass = (): ElementArrayFinder =>
                    comboboxWithTemplate.getElement().all(by.css(".nui-overlay .combobox-examples-custom-template"));
        ...
        xdescribe("appendToBody >", () => {
            it("detached popup menu should be opened and appended to body", async () => {
                ...
                expect(await comboboxAppendToBody.popup.isOpenedAppendToBody()).toBe(true);
            });

            it("detached popup menu should have same width as combo's container", async () => {
                await comboboxAppendToBody.toggleMenu();
                const popupSize = await comboboxAppendToBody.popup.getPopupBoxDetachedArea().getSize();
                const comboSize = await comboboxAppendToBody.getContainer().getSize();
        ...
        describe("removeValueButton >", () => {
            it("should display removeValue button when text is typed", async () => {
                expect(await comboboxRemoveValueButton.clearButton.isPresent()).toBeFalsy();
                const inputText = "Some text";
                await comboboxRemoveValueButton.acceptText(inputText);
                expect(await comboboxRemoveValueButton.clearButton.isPresent()).toBeTruthy();
            });
