    ...
    */
    ...
    ngOnInit(): void {
        ...
        this.datePicker.setRefreshViewHandler((): void => {
            ...
            const firstDateOfMonthDayOfWeek = value.clone().set("date", 1).day();
            ...
            const firstDateOfMonth = (numDisplayedFromPreviousMonth > 0)
                ? value.clone().set("date", (-numDisplayedFromPreviousMonth + 1))
            ...
            for (let i = 0; i < 42; i++) {
                const handler = this.datePicker.handleTimezone ? moment.parseZone : moment;
        ...
        this.datePicker.setCompareHandler((date1: Moment, date2: Moment): number => {
            const d1 = moment({"year": date1.year(), "month": date1.month(), "date": date1.date()});
            const d2 = moment({"year": date2.year(), "month": date2.month(), "date": date2.date()});
