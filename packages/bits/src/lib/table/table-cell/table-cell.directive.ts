...
import { ChangeDetectorRef, Directive, ElementRef, HostBinding, HostListener, Input, OnChanges, OnDestroy, OnInit, SimpleChanges } from "@angular/core";
...
export class TableCellDirective extends CdkCell implements OnInit, OnDestroy, OnChanges {
    ...
    ngOnInit() {
        ...
        if (this.tableStateHandlerService.reorderable) {
            this.subscribeToDraggedOverCell = this.tableStateHandlerService.draggedOverCell.subscribe((draggedOverCell: DraggedOverCell) => {
        ...
        if (this.tableStateHandlerService.resizable) {
            this.resizeSubscription = this.tableStateHandlerService.shouldHighlightEdge
                .pipe(
                    filter(value => value.columnIndex === this.currentCellIndex)
                )
                .subscribe(() => {
