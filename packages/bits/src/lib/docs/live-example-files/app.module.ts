export const APP_MODULE = (filenamePrefix: string, context: string, imports: string, mainComponentName: string, componentNames: string, customRoutes: boolean, chartsImport?: boolean, dashboardsImport?: boolean) =>
    ...
    `        RouterModule.forRoot(${ customRoutes ? getCustomRoute(context) : getDefaultRoute(context, mainComponentName) })\n` +
    "    ],\n" +
    "    providers: [\n" +
...
const getDefaultRoute = (context: string, mainComponentName: string): string => {
    return `[{path: "", redirectTo: "${context}",pathMatch: "full"},\n` +
