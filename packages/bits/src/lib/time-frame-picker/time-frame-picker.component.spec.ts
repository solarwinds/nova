    ...
    describe("time frame picker inline >", () => {
        ...
        describe("when selectPreset() is executed", () => {
            it("it set model based on selected preset", () => {
                // act
                componentInstance.selectPreset("last12Hours", <ITimeFramePreset> {
                    name: "Last 12 hours",
                ...
                // assert
                expect(componentInstance.model.selectedPresetId).toEqual("last12Hours");
                expect(componentInstance.model.startDatetime).toBeDefined();
                expect(componentInstance.model.endDatetime).toBeDefined();
                expect(moment(componentInstance.model.startDatetime)
                    .isSame(moment(componentInstance.model.endDatetime).add({hours: -12}), "second")).toBe(true);
            });
        });
        ...
        describe("when onChangeInternal() is executed", () => {
            it("it should clear 'selectedPresetId' and 'title' if any preset was used", () => {
                ...
                // act
                componentInstance.onChangeInternalStart(componentInstance.model.startDatetime);

                // assert
                expect(componentInstance.model.title).toBeUndefined();
                expect(componentInstance.model.selectedPresetId).toBeUndefined();
            });
        });
        ...
        describe("when start-date set is changed", () => {
            it("to later than end-date it updates end-date keeping the previous distance without any preset", () => {
                ...
                componentInstance.onChangeInternalEnd(componentInstance.model.endDatetime);
                expect(componentInstance.distanceToEndDate / 3600000).toEqual(25); // each hour has 3600000 ms

                // change the startDate to be startDate + 48 hours
                componentInstance.model.startDatetime = baseDate.clone().add(48, "hours");
                componentInstance.onChangeInternalStart(componentInstance.model.startDatetime);

                expect(componentInstance.distanceToEndDate / 3600000).toEqual(25); // each hour has 3600000 ms
                expect(componentInstance.model.endDatetime.isSame(baseDate.add(48 + 25, "hours"))).toBe(true);
            ...
            it("to later than end-date it should clear 'selectedPresetId' and 'title' if any preset was used", () => {
                componentInstance.selectPreset("last24Hours", <ITimeFramePreset> {
                    name: "Last 24 hours",
                    startDatetimePattern: {hours: -24},
                    endDatetimePattern: {},
                });

                expect(componentInstance.model.selectedPresetId).toEqual("last24Hours");
                expect(componentInstance.model.startDatetime).toBeDefined();
                expect(componentInstance.model.endDatetime).toBeDefined();
                expect(moment(componentInstance.model.startDatetime)
                    .isSame(moment(componentInstance.model.endDatetime).add({hours: -24}), "second")).toBe(true);

                // change the startDate to be endDatetime + 48 hours
                componentInstance.model.startDatetime = moment(componentInstance.model.endDatetime).add(48, "hours");
                componentInstance.onChangeInternalStart(componentInstance.model.startDatetime);
            ...
            it("to exactly the end-date it updates end-date keeping the previous distance", () => {
                ...
                componentInstance.model.startDatetime = componentInstance.model.endDatetime;
                componentInstance.onChangeInternalStart(componentInstance.model.startDatetime);

                expect(componentInstance.distanceToEndDate / 3600000).toEqual(25); // each hour has 3600000 ms
                expect(componentInstance.model.endDatetime.isSame(baseDate.add(25 + 25, "hours"))).toBe(true);
            });
            it("to before the end-date it updates the distance between both dates", () => {
                ...
                expect(componentInstance.distanceToEndDate / 3600000).toEqual(48 + 25); // each hour has 3600000 ms
                expect(componentInstance.model.endDatetime.isSame(baseDate.clone().add(25, "hours"))).toBeTruthy();
            });
        });
        ...
        describe("deleting input value", () => {
            it("should shift end date after removing and re-selecting start value", fakeAsync(() => {
                ...
                const startDatePicker = debugElement.query(By.css(".nui-time-frame-picker__date-time_start nui-date-time-picker"));
                const startDatePickerInput = startDatePicker.query(By.css("nui-textbox input"));
                ...
                expect(componentInstance.model.startDatetime.format("YYYY-MM-DD")).toEqual("2019-08-28");
                expect(componentInstance.model.endDatetime.format("YYYY-MM-DD")).toEqual("2019-09-01");
            }));
        });
    });
});
