...
class ViewportInRepeatComponent implements AfterViewInit, OnDestroy {
    ...
    public ngAfterViewInit(): void {
        this.nextPage$ = this.viewportManager.setViewport(this.repeat.viewportRef)
            .observeNextPage$({pageSize: this.pageSize}).pipe(shareReplay(1));
...
describe("services >", () => {
    describe("VirtualViewportManager on Repeat > ", () => {
        ...
        it("should throw when trying to reassign viewport", () => {
            expect(() => component.viewportManager.setViewport(component.repeat.viewportRef)).toThrow();
        });
        ...
        it("should load second page", (done: any) => {
            ...
            component.repeat.viewportRef.scrollToIndex(component.repeat.viewportRef.getDataLength());
            component.items$.pipe(skip(1), take(1)).subscribe((r) => {
                expect(component.items$.getValue().length).toEqual(component.pageSize * 2);
                done();
            });
        });
        ...
        it(`should load 5 pages`, (done: any) => {
            ...
            component.items$.pipe(
                // Scrolling 5 pages
                tap(() => {
                    fixture.detectChanges();
                    // Scrolling to last rendered element to trigger next page event
                    component.repeat.viewportRef
                        .scrollToIndex(component.repeat.viewportRef.getDataLength());
                }),
                // Taking last emission to perform check
                skip(pagesToScroll - 1),
                take(1),
                tap((users) => {
                    expect(users.length).toEqual(component.pageSize * pagesToScroll);
                    done();
                })
            ).subscribe();
        });
        ...
        it(`should not try to load more data if the last loaded page is not complete`, (done) => {
            ...
            component.items$.pipe(
                ...
                tap(() => {
                    ...
                    const pagesLoaded = (loadedItems - rest) / component.pageSize;
