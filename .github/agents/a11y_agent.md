...
- **`<a>` vs `<button>` vs `<checkbox>` keyboard activation:**
  ...
  - **Always use `keydown` instead of `keyup`** for Space and Enter activation handlers on non-editable controls. For buttons and custom non-editable controls, use `(keydown.space)="$event.preventDefault(); handler()"` to prevent the browser from scrolling down. Never prevent the default for a plain Space typed into an editable input; shared handlers must honor their `skipSpace` contract. Adding Space to a link is non-standard and confuses screen reader users.
...
- **Composite widget keyboard navigation (WCAG 2.1.1):** Applies to **any** component that opens an overlay/popup with selectable content from a trigger (icon, button, or input) — date/time/color pickers, dropdowns, comboboxes, autocompletes, menus, or any custom equivalent. Named components (`nui-date-picker`, `nui-color-picker`, `nui-time-picker`, `nui-combobox` dropdown) are only examples. For every such widget, verify:
  ...
  > **Template audit is not enough for composite widgets.** When a template has `(keydown)="onKeyDown($event)"`, the handler delegation chain MUST be followed into the component `.ts` and every referenced service (e.g. `OptionKeyControlService`). A template `(keydown)` binding that appears correct can mask an incorrect key-handling branch in a shared service. Check that non-editable select-like controls support both `ENTER` and `SPACE` for item selection, including `event.preventDefault()` where needed to prevent page scroll.
- **Shared keyboard services and editable inputs (`skipSpace`) (Critical):** When a keyboard service is shared by a non-editable select-like control and an editable input such as a combobox, a `skipSpace` flag (or equivalent) is a complete behavior contract, not only a guard around item selection.
  1. Trace **every** `SPACE` branch through the complete delegation chain, including open-popup and closed-popup handlers. Search for all `KEYBOARD_CODE.SPACE`, `event.code === ...SPACE`, `preventDefault()`, item activation, and popup toggle calls before editing.
  2. When `skipSpace === true`, pressing Space MUST NOT call `preventDefault()`, activate/click an option, change selection, or open/close/toggle the popup. The browser default must remain available so the editable input can insert the literal space, even while its popup is open and an option is active.
  3. When `skipSpace === false`, a non-editable select-like control MAY prevent the Space default and use Space to open the popup or activate the current option, according to its keyboard contract.
  4. Apply the same `!skipSpace` guard to **every Space-specific side effect**. It is forbidden to guard option activation while leaving an unconditional Space `preventDefault()` elsewhere in the handler. For example, `if (event.code === SPACE) preventDefault()` followed by `if (!skipSpace && event.code === SPACE) select()` is incorrect; both effects must honor `skipSpace`.
  5. Add focused regression tests before completing the change. At minimum verify an open popup with an active option in both modes: with `skipSpace === true`, `preventDefault()` and option activation are not called; with `skipSpace === false`, the default is prevented and the option is activated. Also verify the closed-popup path does not toggle the popup when `skipSpace === true`. Add a component-level test that types a value containing spaces into the editable input when the test harness supports real keyboard input.
